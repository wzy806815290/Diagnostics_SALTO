function [segs_obs_opt_all, segs_sim_opt_all, cons_all, e_sd_t_all, e_sd_q_all] = ...
    f_CoarseGraining_Continuous(Cat_id_i, obs, sim, timeseries_splits, weight_nfc, weight_rds, weight_sdt, weight_sdv, error_model)
...
%% pre-processing 
    for i = 1 : size(timeseries_splits,1)-1 % loop over all items in timeseries_splits list
        ...
        txt = ['Cat ' num2str(Cat_id_i) ': time series split ' num2str(i) ' of ' num2str(size(timeseries_splits,1)-1)];
        disp (txt)
        ...
        for z = 1: num_red  % reduce until only 2 or 3 segments are left (2: when started with even # of segments, 3: when started with odd # of segments)
            ...
            for z_obs = 2:length(segs_obs)-1  % loop over all observed segments, except the first and last
                ...
                for z_sim = 2:length(segs_sim)-1 % loop over all simulated segments, except the first and last
                    ...
                    [fdist_q, fdist_t,~,~,~,~,~,~,~,~,~,~] = f_SD(obs,tmp_segs_obs,sim,tmp_segs_sim,error_model, 'true'); 
            ...
            % BUG
            %disp([Cat_id_i ';' num2str(z) ';' num2str(length(norm_tmp_percfalsecase)) ';' num2str(length(norm_tmp_rel_del_seg)) ...
            %    ';' num2str(length(norm_tmp_mafdist_t)) ';' num2str(length(norm_tmp_mafdist_v)) ';' num2str(length(tmp_opt))])
            ...
            % progress info
            txt = ['Cat ' num2str(Cat_id_i) ': reduction step ' num2str(z) ' of ' num2str(num_red)];
            disp (txt)
        end
    ...
    %% Calculate objective function and find the optimal coarse graining step
        ...
        sorted_obF = sort(ObFuncVal);
      
        for opt_i = 1:length(ObFuncVal)
            opt_step = find(ObFuncVal == sorted_obF(opt_i));
            ...
            %% find the optimal step until there are both available rising and falling segments
            if length(e_sd_rise_opt) > 0 && length(e_sd_fall_opt) > 0
                break
            end
        end
        opt_ObF = ObFuncVal(opt_step); 
        ...
        if length(ObFuncVal) > 1  % display best coarse graining step
            ...
            disp(['only single step in the split']);
        end
      
        disp(['optimal ObjFuncVal is ' num2str(opt_ObF)]);
